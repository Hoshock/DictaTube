...
- D1 access inside the Worker is just `env.DB` (passed down as a plain `D1Database` argument to route functions) — query with `db/schema.sql`'s column names (snake_case in SQL, mapped to camelCase `shared/types.ts` shapes by small `toX` mapper functions in each route file).
