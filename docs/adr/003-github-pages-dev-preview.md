...
A one-time manual repository setup **is** required, despite the `pages: write` permission already granted in `gh-pages.yml`: the owner must visit Settings → Pages → Build and deployment → Source and select "GitHub Actions" once. Confirmed by observed run failures — `actions/configure-pages@v5`'s default `Get Pages site` call 404s when Pages was never enabled, and passing `enablement: true` to have it create the site instead fails with `Resource not accessible by integration`, because creating a Pages site via the API needs repo-admin permission that the default `GITHUB_TOKEN`'s `pages: write` scope doesn't include.
